            ...
            <div className='w-full border-t-1 border-[#E8E8E8] h-[120px] lg:h-[140px] flex gap-4'>
                <div className='flex w-full flex-col items-end mt-[10px] lg:mt-[8px] gap-1 lg:gap-2'>
                    <div className='w-full flex justify-between'>
                        <h1 className=' text-right font-[400] text-[7px] sm:text-[9px] lg:text-[12px] text-[#6D6C80] ml-5'>{insertDate} </h1>
                        <h2 className='font-[600] text-[12px] text-[#1C1A4A]'>{author}</h2>
                    </div>

                    <h3 className='font-[600] text-[11px] sm:text-[14px] lg:text-[16px] text-[#1C1A4A]'> {title}</h3>
                    <h4 className=' text-right font-[400] text-[8px] sm:text-[10px] lg:text-[14px] text-[#6D6C80]'>{describe}</h4>
                    <div className='flex w-full justify-between'>
                        ...
                        <div className=' h-[25px] flex flex-row gap-0.5'>
                            <button className ='lg:px-3 text-dark-purple h-[34px] text-[8px] sm:text-[10px] lg:text-[12px] bg-light-purple sm:h-[30px] sm:px-2 px-1.5 rounded-[10px] flex items-center'
                                onClick={toggleReplies}
                            >
                                {showReplies ? " بستن  پاسخ ها" : " مشاهده پاسخ ها"}
                            </button>
                            <ModalSection
                                StyleModal={"lg:px-3 sm:px-2 h-[30px] rounded-[10px] flex items-center bg-light-purple"}
                                isOpen={isModalOpen}
                                onClose={toggleModal}
                                onOpen={toggleModal}
                                size="3xl"
                                ...
                                ButtonText={ 
                                    <span className='font-[500] text-[8px] sm:text-[10px] lg:text-[12px] text-dark-purple'>پاسخ</span>
                                }
                            >
